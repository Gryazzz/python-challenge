...
os.chdir('/Users/sonik/Desktop/BC/GitHub/Python/python-challenge/PyBoss/raw_data/')
filename = os.path.join('employee_data1.csv')
...
with open(filename, 'r', newline='', encoding='latin-1') as csvfile, open(outputpath, 'w', newline='') as matched:
    ...
    # Move the first row from original file to a new one
    #csvwriter.writerow(next(csvreader)) # Unfortunately we need another headings()
    header = next(csvreader) # Consume the first line
